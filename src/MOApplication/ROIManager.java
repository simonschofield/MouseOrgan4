...
public class ROIManager {
	...
	ArrayList<SubROI> SubROIList = new ArrayList<>();
	...
	public ImageCoordinateSystem getCurrentROIImageCoordinateSystem_SessionScaled() {
		...
		if(!isUsingMaster()) {
			...
			ROIEXtentsFullRenderWidth = Math.round(ROIEXtentsFullRenderWidth*scl); // session scaling of the document happens here
	...
	public boolean isUsingMaster() {
		if(currentROIName.contains("master") || currentROIName.contains("Master")) {
			return true;
		}
		return false;
	}
	...
	public boolean isCurrentROI(String currentROI) {
		if(currentROIName.equals(currentROI)) {
			return true;
		}
		return false;
	}
	...
	public ArrayList<String> getROINames(){
		...
		for(SubROI ri: SubROIList) {
			if(ri.name.equals("master") || ri.name.equals("Master")) {
				continue;
			}
...
class SubROI{
	...
	public SubROI(String name, Rect rOIExtents, int pixelWidth) {
		...
		extentsFullPixelHeight = Math.round(extentsFullPixelWidth/aspect);
