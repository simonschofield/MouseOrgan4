...
class FloatImage{
  ...
  public FloatImage(String pathandfile){
	...
	if(pathandfile.endsWith(".data")) loadFloatData(pathandfile);
  }
  ...
  private FloatImage getResizedImageCopy(int newXDim, int newYDim){
    String mode = "";
    if(newXDim < 1) return null;
    if(newXDim == xdim) return null;
  ...
  FloatImage downScaleImage(int newXDim, int newYDim){
      ...
      float blendVal = MOMaths.norm( (float)newXDim, (float)smallerImage.xdim, (float)biggerImage.xdim);
