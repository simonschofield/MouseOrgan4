...
public class CMY_HalftoneImage{
	...
	public CMY_HalftoneImage( BufferedImage srcImage) {
		initialise(   srcImage,  new Rect());
	}
	
	public CMY_HalftoneImage(  BufferedImage srcImage, Rect targetArea) {
		// the target  area is in normalised space, to be more human readable
		initialise(   srcImage,  targetArea);
	}
	
	
	private void initialise( BufferedImage srcImage, Rect targetArea) {
