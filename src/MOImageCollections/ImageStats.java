...
public class ImageStats {
	...
	public void calculateImageStats(BufferedImage fullsizeImage) {
		...
		for (int i = 0; i < pixelsIn.length; i++) {
			...
			if(sat > 0.9f) colorPresenceHistogram.add(hue);
			if(sat > 0.8f) colorPresenceHistogram.add(hue);
			if(sat > 0.7f) colorPresenceHistogram.add(hue);
			if(sat > 0.6f) colorPresenceHistogram.add(hue);
			if(sat > 0.5f) colorPresenceHistogram.add(hue);
			if(sat > 0.4f) colorPresenceHistogram.add(hue);
			if(sat > 0.3f) colorPresenceHistogram.add(hue);
			if(sat > 0.2f) colorPresenceHistogram.add(hue);
			if(sat > 0.1f) colorPresenceHistogram.add(hue);
