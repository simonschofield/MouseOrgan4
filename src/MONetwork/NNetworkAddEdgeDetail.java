...
public class NNetworkAddEdgeDetail {
	...
	public void displacePoints(float amt) {
		...
		for(NPoint np: npointList) {
			...
			if(moveCount%100 == 0) {
				System.out.println("done ..." + moveCount + " out of " + totalPoints);
			}
		}
	...
	boolean isDocumentEdgePoint(NPoint np) {
		...
		for(NEdge e: connectedEdges) {
			if(e.getAttributes().containsEqual(docEdgeKVPL)) {
				return true;
			}
		}
		return false;
	...
	private static ArrayList<NEdge> findCrossingEdges(NEdge thisEdge, ArrayList<NEdge> otherEdges) {
		...
		ArrayList<NEdge> crossingEdges = new ArrayList<>();
		...
		for(NEdge e: otherEdges) {
			...
			if( !thisLine.isIntersectionPossible(otherLine) ) {
				continue;
			}

			if( thisLine.calculateIntersection(otherLine) && !thisLine.isConnected(otherLine)) {
				crossingEdges.add(e);
			}
		}
	...
	private int addDetailToEdge(NEdge e, float minSectionLength, float maxDisplacement, float meanNumSections) {
		...
		if(MOMaths.isBetweenInc(numNewPoints, 1, meanNumSections)) {
			maxDisplacement *= 0.2f;
		}
		if(MOMaths.isBetweenInc(numNewPoints, meanNumSections, meanNumSections*2)) {
			maxDisplacement *= 0.4f;
		}
		if(MOMaths.isBetweenInc(numNewPoints, meanNumSections*2, meanNumSections*4)) {
		...
		ArrayList<PVector> pointsToInsert = new ArrayList<>();
		...
		ArrayList<NPoint> nPointsToDisplace = new ArrayList<>();
		NEdge edgeToSplit = e;
