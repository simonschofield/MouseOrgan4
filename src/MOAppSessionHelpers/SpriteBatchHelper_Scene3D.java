...
public class SpriteBatchHelper_Scene3D {
	 ...
	 * @param appendToCollatedSprites - set to true if this sprite batch is to be added to the collated sprites. Normally set to true unless you are using a sprite batch to be the centre points for clusters.
	 * @return the SpriteBatch with the generated points. docPoint, 3Ddepth and SpriteBatchName, UniqueID, RandomKey
	 */
	public SpriteBatch createSpriteBatch3D(String name, float radius, int pointGeneratorRandomSeed, int spriteRandomKeyStart, int maxNumPoints, boolean appendToCollatedSprites) {
		...
		if(appendToCollatedSprites) collatedSpriteBatches.append(spriteBatch);
		return spriteBatch;
	}
	 ...
	 * This method assumes that dense areas of sprites are in the bright areas of the image, with a maximum packing defined by controlValMax and radAtControlMax. Control values above controlValMax will be clamped, resulting in even distribution.
	 * Dark areas, result in sparse packing, using controlValMin, radAtControlMin. Any values below controlValMin will be excluded from packing, resulting is empty regions. 
	 * @param name - the generated SpriteBatch name. 
	 * @param packingImage - the name of the image in the SceneData3D texture collection used to effect the distribution of points
	 * @param controlValMin - The value in the image at which the packing is radAtControlMin. At lowers values, the radius is EXCLUDED, creating areas with no points. 
	 * @param controlValMax - The value in the image at which the packing is radAtControlMax. At higher values, the points are CLAMPED, creating areas with even distribution. 
	 * @param radAtControlMin - The radius (in 3D units) at the image data controlValMin, where the packing is most thin. So this should be a higher number (greater radius) than radAtControlMax
	 * @param radAtControlMax - The radius (in 3D units) at the image data controlValMax, where the packing is most dense. So this should be a smaller number (less radius) than radAtControlMin
	 * @param pointGeneratorRandomSeed - the random seed for the point-packing generator
	 ...
	 */
	public SpriteBatch createSpriteBatch3D(String name, String packingImage, float controlValMin, float controlValMax, float radAtControlMin, float radAtControlMax, 
										   int pointGeneratorRandomSeed, int spriteRandomKeyStart, int maxNumPoints, boolean appendToCollatedSprites) {
	...
	/**
	 * Once a spritebatch has been created, cluster is added around each point, thereby dramatically increasing the number of sprites. The initial sprite-batch is modified to accommodate the new 
	 * cluster-sprites
	 * @param spriteBatchIn
	 * @param minNumber
	 * @param maxNumber
	 * @param maxRange
	 * @param distributionFactor - set to 1 == evenly spread, set to 2 = slight centering, above 2 more and more centreing
	 * @param appendToCollatedSprites - set to true if this sprite batch is to be added to the collated sprites. Normally set to true unless you are using a sprite batch to be the centre points for clusters.
	 * @return
	 */
	public SpriteBatch addClustersToSpriteBatch(SpriteBatch spriteBatchIn, int minNumber, int maxNumber, float maxRange, int distributionFactor, int SpriteRandomKeyStart, int clusterRandomSeed, boolean appendToCollatedSprites) {
		//SpriteBatch spriteBatchOut = new SpriteBatch(spriteBatchIn.thisSpriteBatchName);
		
		ArrayList<PVector> clusterCentreDocPoints = spriteBatchIn.getPoints();

		int spriteRandomKey = SpriteRandomKeyStart;
		int initialNumber = spriteBatchIn.getNumItems();
		System.out.print("Generating " + initialNumber + " Clusters for " + currentSpriteBatchName + " ");
		for(PVector clusterCentreDocPoint: clusterCentreDocPoints) {
			
			
			ArrayList<PVector> clusterDocPoints = generateCluster(clusterCentreDocPoint, minNumber,  maxNumber,   maxRange, distributionFactor, clusterRandomSeed++);
			
			for(PVector p: clusterDocPoints) {
				...
				// Cluster information is added to each sprite: The offset in true 3D units from the cluster-centre-point and the 
				// normalised distance from the cluster centre, where 0 is "no distance from centre point", and 1 is "max distance from centre point"
				float distFromCentre = clusterCentreDocPoint.dist(p);
				float normalisedDistFromCentre = MOMaths.MOMaths.norm(distFromCentre,0,maxRange);
				PVector offsetFromCentre = PVector.sub(p, clusterCentreDocPoint);
				kvpl.addKeyValue("ClusterOffsetFromCentre", offsetFromCentre.array());
				kvpl.addKeyValue("ClusterNormalisedDistance", normalisedDistFromCentre);
				
				
				
				sprite.setSpriteData(kvpl);
				//System.out.println( sprite.getSpriteDataAsCSVLine(null) );
				spriteBatchIn.addSprite(sprite);
			}
			
		}
		if(appendToCollatedSprites)  collatedSpriteBatches.append(spriteBatchIn);
		System.out.println(" added a total of  " + (spriteBatchIn.getNumItems()-initialNumber) + " sprites to make the clusters " );
		return spriteBatchIn;
		
		
		
	}
	
	private ArrayList<PVector> generateCluster(PVector clusterCentreDocPoint, int minNumber, int maxNumber, float maxRange, int distributionFactor, int clusterRandomSeed) {
		if(distributionFactor < 1) distributionFactor = 1;
		
		QRandomStream ranSream = new QRandomStream(clusterRandomSeed);
		
		
		PVector clusterCentre3D = GlobalSettings.getSceneData3D().get3DSurfacePoint(clusterCentreDocPoint);
		PVector surfaceNormal3D = GlobalSettings.getSceneData3D().getSurfaceNormal(clusterCentreDocPoint);
		
		int numPoints = ranSream.randRangeInt(minNumber, maxNumber);
		ArrayList<PVector> clusterPoints = new ArrayList<PVector>();
		
		for(int n = 0; n < numPoints; n++) {
		
			// generating a random point around the centre point 3d
			// generate a random 3D vector, normalise it, create a distance from the centre, and crate a point at this.
			// Then use the surface normal to re-project onto the 3D surface 
			
			// generate a random 3D vector
			float rvx = ranSream.randRangeF(-1, 1);
			float rvy = ranSream.randRangeF(-1, 1);
			float rvz = ranSream.randRangeF(-1, 1);
			PVector offsetvector = new PVector(rvx, rvy, rvz);
			offsetvector.normalize();
			
			// create a slightly biased distance, where outliers are less common (average of 2 samples)
			float distance = Math.abs(ranSream.randRangeAveragedDistribution(-maxRange, maxRange, distributionFactor));
			
			// multiply the random vector by the distance and add to the centre point.
			// YOu now have a random point which is offset from the centre point
			offsetvector.mult(distance);
			PVector displacePoint = PVector.add(clusterCentre3D, offsetvector);
			
			// Project this point onto the surface, along a ray which whose vector is the centre point normal
			Ray3D ray = new Ray3D(displacePoint, surfaceNormal3D);
			PVector surfacePoint3D = GlobalSettings.getSceneData3D().raySurfaceIntersection(ray);
			
			// calculate the doc point, and return a depth-enhanced docPoint
			PVector docPoint = GlobalSettings.getSceneData3D().world3DToDocSpace(surfacePoint3D);
			docPoint.z = GlobalSettings.getSceneData3D().getDepth(docPoint);
			clusterPoints.add(docPoint);
	...
	public void saveCollatedSpriteBatch(String csvFname) {
		collatedSpriteBatches.saveSpriteBatch( collatedSpriteBatches.getSpriteBatchDirectoryPath() + "//" + csvFname, null );
	}
