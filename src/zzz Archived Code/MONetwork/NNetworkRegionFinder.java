...
//
 ...
 public class NNetworkRegionFinder{
	...
	ArrayList<NEdge> startEdgeList = new ArrayList<>();
	...
	public void findAllRegions() {
		...
		ArrayList<NRegion> bigRegions = new ArrayList<>();
	...
	public ArrayList<String> getListRegionTypes() {
		...
		ArrayList<String> regiontypes =  new ArrayList<>();
		for (NRegion nr : theNetwork.regions) {
			String s = nr.getAttributeStringVal("REGIONTYPE");
			if(!regiontypes.contains(s)) {
				regiontypes.add(s);
			}
		}
		return regiontypes;
	...
	boolean findRegion(int pass) {
		...
		ArrayList<NEdge> thisRegionEdges = new ArrayList<>();
	...
	NEdge getStartEdge(int pass) {
		if(pass==0) {
			return getVirginEdge();
		}

		if(pass==1) {
			return getEdgeFromStartEdgeList();
		}

		if(pass==2) {
			return getRemainingStartEdgeWith1ARC();
		}
	...
	NEdge getVirginEdge() {
		...
		for(NEdge e: theNetwork.edges) {
			if( e.getAssociatedRegionCount()==0 && !startEdgeList.contains(e) ) {
				startEdgeList.add(e);
				return e;
			}
		}
	...
	NEdge getRemainingStartEdgeWith1ARC() {
		...
		for(NEdge e: theNetwork.edges) {
			if( e.getAssociatedRegionCount()==1 && !startEdgeList.contains(e)) {
				startEdgeList.add(e);
				return e;
			}
		}
	...
	private NEdge getMostClockwiseConnectedEdge(NEdge thisEdge, NPoint usingThisPoint) {
		...
		if(connectedEdges==null) {
			System.out.println("getMostClockwiseConnectedEdge:NULL1");
		}
		if(thisEdge==null) {
			System.out.println("getMostClockwiseConnectedEdge:NULL2");
		}
	...
	private void removeAllDanglingEdges() {
		while(true) {
			...
			if(danglingEdgesFoundAndRemoved==0) {
				break;
			}
		}
	}
	...
	boolean isDanglingEdge(NEdge e) {
		...
		if(numConnectedP1 == 1 || numConnectedP2 == 1) {
			return true;
		}
		return false;
